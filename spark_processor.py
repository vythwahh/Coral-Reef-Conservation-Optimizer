...
# ─────────────────────────────────────────────
# SCHEMA — khớp với message từ Kafka Producer
# ─────────────────────────────────────────────
...
# ─────────────────────────────────────────────
# UDF: TÍNH RISK SCORE TỔNG HỢP
# ─────────────────────────────────────────────

def compute_risk_score(sst, bleaching_risk):
    """
    Risk score tổng hợp từ SST và bleaching risk.
    Công thức:
...
def read_from_kafka(spark):
    """
    Đọc data từ Confluent Cloud Kafka topic.
    Dùng SASL_SSL để authenticate.
    """
    jaas_config = (
...
def process_data(raw_df):
    """
    Parse JSON từ Kafka value → tính risk score → aggregate.
    """
    ...
    scored_df = (
        ...
        .withColumn(
            "risk_score",
            risk_score_udf(col("sst_celsius"), col("bleaching_risk"))
        )
        .withColumn("processed_at", current_timestamp())
    )

    # Aggregate: lấy trung bình theo reef_id
    aggregated_df = (
        scored_df
...
def main():
    print("🚀 Khởi động Spark Processor...")
    spark = create_spark_session()
    spark.sparkContext.setLogLevel("WARN")

    print("📡 Đọc data từ Kafka topic: coral-raw-data...")
    raw_df = read_from_kafka(spark)

    print("⚙️  Xử lý và tính risk score...")
    scored_df, aggregated_df = process_data(raw_df)

    print("\n📊 Risk Score từng vùng san hô:")
    print("=" * 60)
