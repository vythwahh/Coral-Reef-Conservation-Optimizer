...
def fetch_sst(lat, lon):
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            print(f"  NOAA API error {resp.status_code} → falling back to mock data")
            return None
    except Exception as e:
        print(f"  NOAA connection error: {e} → falling back to mock data")
        return None


def mock_sst(lat):
    """Generate realistic mock SST based on latitude."""
    import numpy as np
...
# BLEACHING RISK CALCULATION
 

def calc_bleaching_risk(sst):
    """
    Calculate bleaching risk based on NOAA Coral Reef Watch formula.
    SST <= 29°C → risk = 0.0
    SST 29-31°C → risk increases linearly
    SST >= 31°C → risk = 1.0
    """
    if sst <= 29.0:
        return 0.0
...
def create_producer():
    """Create Kafka producer with Confluent Cloud credentials."""
...
def produce_data():
    ...
    print(f" Kafka Producer connected to Confluent Cloud")
    print(f" Streaming NOAA data to topic '{KAFKA_TOPIC}'\n")
