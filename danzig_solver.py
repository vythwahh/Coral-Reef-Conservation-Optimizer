...
class DantzigSolver:
    """
    Single-phase Simplex using Dantzig's Rule.

    Requirements: BFS already available (all constraints are <=).
    For problems with >= or = constraints, use TwoPhaseSolver instead.
    Difference from Bland: entering variable = most negative reduced cost.
    """
    ...
    def _simplex(self, tableau, basis, m, n_vars, history):
        for iteration in range(1000):
            ...
            if min_ratio == float('inf'):
                current_step["message"] = "Unbounded."
                history.append(current_step)
                return "Unbounded", None, None, history

            # Tiebreak: smallest basis index
            ...
            current_step["message"] = (
                f"x{entering_col+1} enters (coeff {tableau[-1, entering_col]:.4f}), "
